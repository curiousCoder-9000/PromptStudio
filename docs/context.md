...
promptstudio/
  ...
  scraping/
    ...
    downloader.py        # saved / creator / following sync
    filters.py           # bio keywords, min media, public-only
    queue.py             # following_queue.json daily budget
    checkpoints.py       # sync_state.json telemetry
    sync_manager.py      # single background sync job
...
| GET | `/api/prompt/batch/status` | batch progress |
| PUT | `/api/favorite` | toggle / set |
| DELETE | `/api/photo` | disk + cache + meta + fav |
...
1. **Never delete archive media** without user confirm via UI `#deleteConfirmModal` → `DELETE /api/photo`.
