...
keep/reject verdicts that cost GPU hours, favourites that are the user's own
judgement, creator styles, and perceptual hashes.
...
# Order matters on import — prompts first so a restored archive is coherent
# at every intermediate step if the run is interrupted.
DERIVED_KINDS = (
    "prompts",
    "favorites",
    "styles",
...
# The kinds backed directly by a table. `favorites` and `styles` are
# file-backed and each has a branch in `_collect` / `_apply` instead.
_TABLE_FOR_KIND = {
    "prompts": "prompts",
