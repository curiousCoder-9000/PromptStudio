...
def test_single_tier_correction_shape(api, make_photo):
    index = ArchiveIndex.get()
...
def test_bad_tier_is_400(api, make_photo):
    index = ArchiveIndex.get()
    rels = _seed(index, make_photo, [("a.jpg", 2)])
    status, _payload = api(
        "POST", "/api/classify/verdict", {"rel_path": rels["a.jpg"], "tier": 9}
    )
    assert status == 400
    status, _payload = api(
        "POST", "/api/classify/verdict", {"rel_path": rels["a.jpg"]}
    )
    assert status == 400
...
def test_clearing_the_gold_label_via_the_api(api, make_photo):
    ...
    assert status == 200
    assert payload["verdict"]["corrected_tier"] is None
    assert payload["verdict"]["tier"] == 2


def test_bulk_tier_correction(api, make_photo):
    index = ArchiveIndex.get()
...
def test_photos_accepts_the_disagreement_filter(api, make_photo):
    ...
    assert status == 200
    assert payload["total"] == 1
    assert payload["photos"][0]["rel_path"] == rels["miss.jpg"]
